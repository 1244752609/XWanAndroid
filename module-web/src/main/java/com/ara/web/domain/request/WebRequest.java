package com.ara.web.domain.request;
...
import com.ara.web.data.repository.DataRepository;
import com.kunminx.architecture.ui.callback.UnPeekLiveData;

/**
 * Created by XieXin on 2022/3/4.
 */
public class WebRequest extends BaseRequest implements DefaultLifecycleObserver {
    // 让 Request 可观察页面生命周期，
    // 从而在页面即将退出、且登录请求由于网络延迟尚未完成时，
    // 及时通知数据层取消本次请求，以避免资源浪费和一系列不可预期的问题。
    private final UnPeekLiveData<DataResult<BaseBean<Object>>> webLiveData = new UnPeekLiveData<>();
    ...
    public UnPeekLiveData<DataResult<BaseBean<Object>>> getWebLiveData() {
    ...
    public void web(String name) {
        DataRepository.getInstance().web(name, webLiveData::postValue);
    }
