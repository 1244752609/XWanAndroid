package com.ara.module_public.domain.request;
...
import com.ara.module_public.data.repository.DataRepository;
 ...
 */
public class PublicRequest extends BaseRequest implements DefaultLifecycleObserver {
    // 让 Request 可观察页面生命周期，
    // 从而在页面即将退出、且登录请求由于网络延迟尚未完成时，
    // 及时通知数据层取消本次请求，以避免资源浪费和一系列不可预期的问题。
    private final UnPeekLiveData<DataResult<BaseBean<List<PublicAuthorBean>>>> authorLiveData = new UnPeekLiveData<>();
    private final UnPeekLiveData<DataResult<BaseBean<PagingBean<ArticleBean>>>> publicLiveData = new UnPeekLiveData<>();
    ...
    public UnPeekLiveData<DataResult<BaseBean<List<PublicAuthorBean>>>> getAuthorLiveData() {
    ...
    public UnPeekLiveData<DataResult<BaseBean<PagingBean<ArticleBean>>>> getPublicLiveData() {
        return publicLiveData;
    }
    ...
    public void getAuthor() {
        DataRepository.getInstance().getAuthor(authorLiveData::postValue);
    }

    public void getListPublic(int page, String cid) {
        DataRepository.getInstance().getListPublic(page, cid, publicLiveData::postValue);
    }
