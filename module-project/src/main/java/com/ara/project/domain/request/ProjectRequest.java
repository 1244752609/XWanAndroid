package com.ara.project.domain.request;
...
import com.ara.project.data.repository.DataRepository;
 ...
 */
public class ProjectRequest extends BaseRequest implements DefaultLifecycleObserver {
    // 让 Request 可观察页面生命周期，
    // 从而在页面即将退出、且登录请求由于网络延迟尚未完成时，
    // 及时通知数据层取消本次请求，以避免资源浪费和一系列不可预期的问题。
    private final UnPeekLiveData<DataResult<BaseBean<List<ProjectClassifyBean>>>> projectTypeLiveData = new UnPeekLiveData<>();
    private final UnPeekLiveData<DataResult<BaseBean<PagingBean<ArticleBean>>>> projectLiveData = new UnPeekLiveData<>();
    ...
    public UnPeekLiveData<DataResult<BaseBean<List<ProjectClassifyBean>>>> getProjectTypeLiveData() {
        ...
        return projectTypeLiveData;
    }

    public UnPeekLiveData<DataResult<BaseBean<PagingBean<ArticleBean>>>> getProjectLiveData() {
    ...
    public void getProjectType() {
        DataRepository.getInstance().getProjectType(projectTypeLiveData::postValue);
    }

    public void getListProject(int page, String cid) {
        DataRepository.getInstance().getListProject(page, cid, projectLiveData::postValue);
    }
