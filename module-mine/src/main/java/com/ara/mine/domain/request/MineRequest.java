 ...
 */
public class MineRequest extends BaseRequest implements DefaultLifecycleObserver {
    // 让 Request 可观察页面生命周期，
    // 从而在页面即将退出、且登录请求由于网络延迟尚未完成时，
    // 及时通知数据层取消本次请求，以避免资源浪费和一系列不可预期的问题。
    private final UnPeekLiveData<DataResult<BaseBean<UserInfoBean>>> userInfoLiveData = new UnPeekLiveData<>();
    private final UnPeekLiveData<DataResult<BaseBean<PagingBean<IntegralBean>>>> integralListLiveData = new UnPeekLiveData<>();
    private final UnPeekLiveData<DataResult<BaseBean<PagingBean<IntegralBean>>>> rankLiveData = new UnPeekLiveData<>();
    private final UnPeekLiveData<DataResult<BaseBean<Object>>> logoutLiveData = new UnPeekLiveData<>();
    ...
    public UnPeekLiveData<DataResult<BaseBean<UserInfoBean>>> getUserInfoLiveData() {
    ...
    public UnPeekLiveData<DataResult<BaseBean<PagingBean<IntegralBean>>>> getIntegralListLiveData() {
        return integralListLiveData;
    }

    public UnPeekLiveData<DataResult<BaseBean<PagingBean<IntegralBean>>>> getRankLiveData() {
        return rankLiveData;
    }
    ...
    public void getUserInfo() {
        DataRepository.getInstance().getUserInfo(userInfoLiveData::postValue);
    }

    public void getIntegralList(int page) {
        DataRepository.getInstance().getIntegralList(page, integralListLiveData::postValue);
    }

    public void getRank(int page) {
        DataRepository.getInstance().getRank(page, rankLiveData::postValue);
    }
